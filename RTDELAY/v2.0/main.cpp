...
/*
	...
	Footer:
		slider1 (time cursor)
		button10 (pause/resume playback)
...
#define PBRUN_CHILDWNDINDEX_BUTTON_PAUSERESUMEPB CHILDWNDINDEX_BUTTON10
...
#define PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR CHILDWNDINDEX_SLIDER1
...
static VOID WINAPI paintscreen_playback_running(VOID);
static VOID WINAPI paintscreen_playback_finished(VOID);
...
static BOOL WINAPI create_childwnd(VOID)
{
	...
	pp_childwnd[PBRUN_CHILDWNDINDEX_BUTTON_PAUSERESUMEPB] = CreateWindow(TEXT("BUTTON"), NULL, style, 0, 0, 0, 0, p_mainwnd, NULL, p_instance, NULL);
	...
	pp_childwnd[PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR] = CreateWindow(TRACKBAR_CLASS, NULL, style, 0, 0, 0, 0, p_mainwnd, NULL, p_instance, NULL);
	...
	while(catch_messages())
	{
		switch(runtime_status)
		{
			case RUNTIME_STATUS_IDLE:
				if((prev_status == RUNTIME_STATUS_PLAYBACK_RUNNING) && (timecursor_status == TIMECURSOR_STATUS_IDLE))
				{
					_audiodatapos = p_audio->getAudioDataPositionFrames();
					SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR], TBM_SETPOS, (WPARAM) TRUE, (LPARAM) _audiodatapos);
				}
...
static VOID WINAPI paintscreen_playback_running(VOID)
{
	...
	SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_BUTTON_PAUSERESUMEPB], WM_SETTEXT, 0, (LPARAM) TEXT("Pause Playback"));
	...
	if(_audiodatasize > 0)
	{
		SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR], TBM_SETRANGEMIN, (WPARAM) FALSE, 0);
		SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR], TBM_SETRANGEMAX, (WPARAM) FALSE, (LPARAM) _audiodatasize);
		SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR], TBM_SETPOS, (WPARAM) FALSE, 0);
		ShowWindow(pp_childwnd[PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR], SW_SHOW);
	}
	...
	ShowWindow(pp_childwnd[PBRUN_CHILDWNDINDEX_BUTTON_PAUSERESUMEPB], SW_SHOW);
...
static VOID WINAPI mainwndchildren_align(VOID)
{
	...
	INT button2_dim[4] = {0};
	INT button3_dim[4] = {0};
	INT button_pauseresumepb_dim[4] = {0};
...
_l_mainwndchildren_align_playback_running:
	...
	button_pauseresumepb_dim[2] = button1_dim[2];
	button_pauseresumepb_dim[3] = BUTTON_HEIGHT;
	button_pauseresumepb_dim[1] = button1_dim[1];

	button_pauseresumepb_dim[0] = mainwnd_centerx - button_pauseresumepb_dim[2] - PBRUN_MARGIN/2;
	button1_dim[0] = mainwnd_centerx - PBRUN_MARGIN/2;

	slider_timecursor_dim[3] = 30;
	slider_timecursor_dim[2] = button_pauseresumepb_dim[2] + button1_dim[2] + PBRUN_MARGIN;
	slider_timecursor_dim[0] = mainwnd_centerx - slider_timecursor_dim[2]/2;
	slider_timecursor_dim[1] = button1_dim[1] - slider_timecursor_dim[3] - PBRUN_MARGIN;

	containerfb_dim[0] = text1_dim[0];
	containerfb_dim[2] = text1_dim[2];
	containerfb_dim[3] = 180;
	containerfb_dim[1] = slider_timecursor_dim[1] - containerfb_dim[3] - PBRUN_MARGIN;
	...
	SetWindowPos(pp_childwnd[PBRUN_CHILDWNDINDEX_BUTTON_PAUSERESUMEPB], NULL, button_pauseresumepb_dim[0], button_pauseresumepb_dim[1], button_pauseresumepb_dim[2], button_pauseresumepb_dim[3], 0u);
	SetWindowPos(pp_childwnd[PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR], NULL, slider_timecursor_dim[0], slider_timecursor_dim[1], slider_timecursor_dim[2], slider_timecursor_dim[3], 0u);
...
_l_mainwnd_event_wmcommand_playback_running:
	...
	else if(((ULONG_PTR) lparam) == ((ULONG_PTR) pp_childwnd[PBRUN_CHILDWNDINDEX_BUTTON_PAUSERESUMEPB]))
	{
		_numptr = (LONG_PTR) p_audio->getStatus();

		switch(_numptr)
		{
			case p_audio->STATUS_PLAYING:
...
static LRESULT CALLBACK mainwnd_event_wmhscroll(HWND p_wnd, WPARAM wparam, LPARAM lparam)
{
	...
	if(((ULONG_PTR) lparam) == ((ULONG_PTR) pp_childwnd[PBRUN_CHILDWNDINDEX_SLIDER_TIMECURSOR]))
	{
...
static VOID WINAPI fxparams_text_update(VOID)
{
	...
	{
		n_delay = (ULONG_PTR) p_audio->rtdelayGetFFDelay(n_fx);
	...
	{
		n_delay = (ULONG_PTR) p_audio->rtdelayGetFBDelay(n_fx);
