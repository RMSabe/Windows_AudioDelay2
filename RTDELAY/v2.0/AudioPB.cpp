...
BOOL WINAPI AudioPB::setParameters(const audiopb_params_t *p_params)
{
	...
	if(this->status > 0)
	{
		this->err_msg = TEXT("AudioPB::setParameters: Error: cannot set parameters, audio object already initialized.");
		return FALSE;
	}
...
LONG64 WINAPI AudioPB::getAudioDataSizeFrames(VOID)
{
	...
	_bytes_per_frame = (this->FILE_BYTES_PER_SAMPLE)*((ULONG_PTR) this->N_CHANNELS);
...
BOOL WINAPI AudioPB::loadAudioDeviceList(VOID)
{
	...
	this->audiodevicelist_n_entries = (ULONG_PTR) devcoll_count;
	...
	for(n_dev = 0u; n_dev < devcoll_count; n_dev++)
	{
		...
		if(propvar.vt == VT_EMPTY) __SPRINTF((TCHAR*) (((ULONG_PTR) this->p_audiodevicelist) + audiodevicelist_byteoffset), this->AUDIODEVICELIST_ENTRYLENGTH, TEXT("Unknown Audio Device"));
		else cstr_copy_wchar_to_tchar(propvar.pwszVal, (TCHAR*) (((ULONG_PTR) this->p_audiodevicelist) + audiodevicelist_byteoffset), this->AUDIODEVICELIST_ENTRYLENGTH);
...
{
	return (LONG_PTR) this->audiodevicelist_n_entries;
}
...
const TCHAR* WINAPI AudioPB::getAudioDeviceListEntry(ULONG_PTR index)
{
	...
	return &(this->p_audiodevicelist[index*(this->AUDIODEVICELIST_ENTRYLENGTH)]);
}
...
LONG_PTR WINAPI AudioPB::rtdelayGetFFDelay(ULONG_PTR n_fx)
{
...
FLOAT WINAPI AudioPB::rtdelayGetFFAmplitude(ULONG_PTR n_fx)
{
	audiortdelay_fx_params_t _params;
...
LONG_PTR WINAPI AudioPB::rtdelayGetFBDelay(ULONG_PTR n_fx)
{
	audiortdelay_fx_params_t _params;
...
FLOAT WINAPI AudioPB::rtdelayGetFBAmplitude(ULONG_PTR n_fx)
{
	audiortdelay_fx_params_t _params;
...
BOOL WINAPI AudioPB::audiodevicelist_alloc(VOID)
{
	if(!this->audiodevicelist_free()) return FALSE;

	if(!this->audiodevicelist_n_entries)
	{
		this->err_msg = TEXT("AudioPB::audiodevicelist_alloc: Error: invalid device entry count value.");
		return FALSE;
	}

	if(p_processheap == NULL)
	{
		this->err_msg = TEXT("AudioPB::audiodevicelist_alloc: Error: p_processheap is NULL.");
		return FALSE;
	}

	this->p_audiodevicelist = (TCHAR*) HeapAlloc(p_processheap, HEAP_ZERO_MEMORY, (this->audiodevicelist_n_entries)*(this->AUDIODEVICELIST_ENTRYLENGTH)*(sizeof(TCHAR)));

	if(this->p_audiodevicelist == NULL)
	{
		this->err_msg = TEXT("AudioPB::audiodevicelist_alloc: Error: failed to allocate heap memory.");
		return FALSE;
	}
...
BOOL WINAPI AudioPB::audiodevicelist_free(VOID)
{
	if(p_processheap == NULL)
	{
		this->err_msg = TEXT("AudioPB::audiodevicelist_free: Error: p_processheap is NULL.");
		return FALSE;
	}

	if(this->p_audiodevicelist != NULL)
	{
		if(!HeapFree(p_processheap, 0u, this->p_audiodevicelist))
		{
			this->err_msg = TEXT("AudioPB::audiodevicelist_free: Error: failed to release heap memory.");
			return FALSE;
		}
