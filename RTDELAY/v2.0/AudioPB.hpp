...
class AudioPB {
	public:
		...
		LONG_PTR WINAPI rtdelayGetFFDelay(ULONG_PTR n_fx);
		FLOAT WINAPI rtdelayGetFFAmplitude(ULONG_PTR n_fx);

		BOOL WINAPI rtdelaySetFFDelay(ULONG_PTR n_fx, ULONG_PTR delay);
		BOOL WINAPI rtdelaySetFFAmplitude(ULONG_PTR n_fx, FLOAT amp);

		LONG_PTR WINAPI rtdelayGetFBDelay(ULONG_PTR n_fx);
		FLOAT WINAPI rtdelayGetFBAmplitude(ULONG_PTR n_fx);
	...
	protected:
		...
		virtual BOOL WINAPI buffer_free(VOID) = 0;

		BOOL WINAPI audiodevicelist_alloc(VOID);
		BOOL WINAPI audiodevicelist_free(VOID);
