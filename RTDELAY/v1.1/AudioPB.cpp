...
BOOL WINAPI AudioPB::rtdelayGetFFParams(SIZE_T n_fx, audiortdelay_fx_params_t *p_params)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioPB::rtdelayGetFBParams(SIZE_T n_fx, audiortdelay_fx_params_t *p_params)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioPB::rtdelaySetFFDelay(SIZE_T n_fx, SIZE_T delay)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioPB::rtdelaySetFFAmplitude(SIZE_T n_fx, FLOAT amp)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioPB::rtdelaySetFBDelay(SIZE_T n_fx, SIZE_T delay)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioPB::rtdelaySetFBAmplitude(SIZE_T n_fx, FLOAT amp)
{
	if(this->status < 1) return FALSE;
...
VOID WINAPI AudioPB::stop_all_threads(VOID)
{
...
VOID WINAPI AudioPB::playback_loop(VOID)
{
	...
	{
		this->p_playthread = thread_create_default(((DWORD (WINAPI *)(VOID*)) &AudioPB::playthread_proc), this, NULL);
		this->p_loadthread = thread_create_default(((DWORD (WINAPI *)(VOID*)) &AudioPB::loadthread_proc), this, NULL);
...
VOID WINAPI AudioPB::buffer_play(VOID)
{
	...
	if(n_ret != S_OK) app_exit(1u, TEXT("AudioPB::buffer_play: Error: IAudioRenderClient::GetBuffer failed."));
...
DWORD WINAPI AudioPB::loadthread_proc(VOID *p_args)
{
	this->buffer_load_in();
...
DWORD WINAPI AudioPB::playthread_proc(VOID *p_args)
{
	this->buffer_play();
	this->audio_hw_wait();
