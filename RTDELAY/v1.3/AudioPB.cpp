...
BOOL WINAPI AudioPB::loadAudioDeviceList(VOID)
{
	...
	{
		this->audio_hw_deinit_all();
		this->err_msg = TEXT("AudioPB::loadAudioDeviceList: Error: memory allocate failed.");
		return FALSE;
	}
...
BOOL WINAPI AudioPB::audiodevicelist_alloc(VOID)
{
	...
	if(!this->audiodevicelist_n_entries) return FALSE;
	...
	return (this->p_audiodevicelist != NULL);
}

VOID WINAPI AudioPB::audiodevicelist_free(VOID)
{
	HANDLE p_processheap = NULL;
	...
	if(this->p_audiodevicelist != NULL)
	{
		HeapFree(p_processheap, 0u, this->p_audiodevicelist);
