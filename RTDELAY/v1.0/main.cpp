...
extern BOOL WINAPI attempt_update_dryamp(VOID);
extern BOOL WINAPI attempt_update_outamp(VOID);
extern BOOL WINAPI attempt_update_ffdelay(VOID);
extern BOOL WINAPI attempt_update_ffamp(VOID);
extern BOOL WINAPI attempt_update_fbdelay(VOID);
extern BOOL WINAPI attempt_update_fbamp(VOID);

extern SSIZE_T WINAPI ffdelay_get_current_chsel(VOID);
extern SSIZE_T WINAPI fbdelay_get_current_chsel(VOID);
...
LRESULT CALLBACK mainwnd_event_wmcommand(HWND p_wnd, WPARAM wparam, LPARAM lparam)
{
	...
	else if(((ULONG_PTR) lparam) == ((ULONG_PTR) pp_childwnd[CHILDWNDINDEX_BUTTON2]))
	{
		if(prev_status == RUNTIME_STATUS_CHOOSEAUDIODEV)
		{
			...
			if(!chooseaudiodev_proc((SIZE_T) _ssize, FALSE)) return 0;
...
BOOL WINAPI attempt_update_dryamp(VOID)
{
	FLOAT f_amp = 0.0f;

	SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_TEXTBOX_UPDATEDRYAMP], WM_GETTEXT, (WPARAM) TEXTBUF_SIZE_CHARS, (LPARAM) textbuf);

	try
	{
		f_amp = std::stof(textbuf);
	}
	catch(...)
	{
		tstr = TEXT("Error: invalid value entered.");
		goto _l_attempt_update_dryamp_error;
	}

	if(!p_audio->rtdelaySetDryInputAmplitude(f_amp))
	{
		tstr = p_audio->getLastErrorMessage();
		goto _l_attempt_update_dryamp_error;
	}
...
_l_attempt_update_dryamp_error:
	MessageBox(NULL, tstr.c_str(), TEXT("ERROR"), (MB_ICONEXCLAMATION | MB_OK));
	return FALSE;
}

BOOL WINAPI attempt_update_outamp(VOID)
{
	FLOAT f_amp = 0.0f;

	SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_TEXTBOX_UPDATEOUTAMP], WM_GETTEXT, (WPARAM) TEXTBUF_SIZE_CHARS, (LPARAM) textbuf);
	...
	{
		tstr = TEXT("Error: invalid value entered.");
		goto _l_attempt_update_outamp_error;
	}

	if(!p_audio->rtdelaySetOutputAmplitude(f_amp))
	{
		tstr = p_audio->getLastErrorMessage();
		goto _l_attempt_update_outamp_error;
	}
...
_l_attempt_update_outamp_error:
	MessageBox(NULL, tstr.c_str(), TEXT("ERROR"), (MB_ICONEXCLAMATION | MB_OK));
	return FALSE;
}

BOOL WINAPI attempt_update_ffdelay(VOID)
{
	SSIZE_T n_fx = -1;
	INT n_delay = 0;
	...
	if(n_fx < 0)
	{
		...
		goto _l_attempt_update_ffdelay_error;
	}

	SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_TEXTBOX_UPDATEFFDELAY], WM_GETTEXT, (WPARAM) TEXTBUF_SIZE_CHARS, (LPARAM) textbuf);
	...
	if(!p_audio->rtdelaySetFFDelay((SIZE_T) n_fx, (SIZE_T) n_delay))
	{
		tstr = p_audio->getLastErrorMessage();
		goto _l_attempt_update_ffdelay_error;
	}
...
_l_attempt_update_ffdelay_error:
	MessageBox(NULL, tstr.c_str(), TEXT("ERROR"), (MB_ICONEXCLAMATION | MB_OK));
	return FALSE;
}

BOOL WINAPI attempt_update_ffamp(VOID)
{
	SSIZE_T n_fx = -1;
	FLOAT f_amp = 0.0f;
	...
	{
		tstr = TEXT("Error: no feedforward delay channel selected.");
		goto _l_attempt_update_ffamp_error;
	}

	SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_TEXTBOX_UPDATEFFAMP], WM_GETTEXT, (WPARAM) TEXTBUF_SIZE_CHARS, (LPARAM) textbuf);
	...
	if(!p_audio->rtdelaySetFFAmplitude((SIZE_T) n_fx, f_amp))
	{
		tstr = p_audio->getLastErrorMessage();
		goto _l_attempt_update_ffamp_error;
	}
...
_l_attempt_update_ffamp_error:
	MessageBox(NULL, tstr.c_str(), TEXT("ERROR"), (MB_ICONEXCLAMATION | MB_OK));
	return FALSE;
}

BOOL WINAPI attempt_update_fbdelay(VOID)
{
	SSIZE_T n_fx = -1;
	INT n_delay = 0;
	...
	if(n_fx < 0)
	{
		...
		goto _l_attempt_update_fbdelay_error;
	}

	SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_TEXTBOX_UPDATEFBDELAY], WM_GETTEXT, (WPARAM) TEXTBUF_SIZE_CHARS, (LPARAM) textbuf);
	...
	if(!p_audio->rtdelaySetFBDelay((SIZE_T) n_fx, (SIZE_T) n_delay))
	{
		tstr = p_audio->getLastErrorMessage();
		goto _l_attempt_update_fbdelay_error;
	}
...
_l_attempt_update_fbdelay_error:
	MessageBox(NULL, tstr.c_str(), TEXT("ERROR"), (MB_ICONEXCLAMATION | MB_OK));
	return FALSE;
}

BOOL WINAPI attempt_update_fbamp(VOID)
{
	SSIZE_T n_fx = -1;
	FLOAT f_amp = 0.0f;
	...
	{
		tstr = TEXT("Error: no feedback delay channel selected.");
		goto _l_attempt_update_fbamp_error;
	}

	SendMessage(pp_childwnd[PBRUN_CHILDWNDINDEX_TEXTBOX_UPDATEFBAMP], WM_GETTEXT, (WPARAM) TEXTBUF_SIZE_CHARS, (LPARAM) textbuf);
	...
	if(!p_audio->rtdelaySetFBAmplitude((SIZE_T) n_fx, f_amp))
	{
		tstr = p_audio->getLastErrorMessage();
		goto _l_attempt_update_fbamp_error;
	}
...
_l_attempt_update_fbamp_error:
	MessageBox(NULL, tstr.c_str(), TEXT("ERROR"), (MB_ICONEXCLAMATION | MB_OK));
	return FALSE;
}

SSIZE_T WINAPI ffdelay_get_current_chsel(VOID)
{
	LRESULT btn_state = 0;
...
SSIZE_T WINAPI fbdelay_get_current_chsel(VOID)
{
	LRESULT btn_state = 0;
...
INT WINAPI filein_get_params(VOID)
{
	...
	while(TRUE)
	{
		...
		if(compare_signature("fmt ", (const CHAR*) (((SIZE_T) p_headerinfo) + bytepos))) break;

		u32 = *((UINT32*) (((SIZE_T) p_headerinfo) + bytepos + 4u));
		bytepos += (SIZE_T) (u32 + 8u);
	}

	u16 = *((UINT16*) (((SIZE_T) p_headerinfo) + bytepos + 8u));
	...
	pb_params.n_channels = *((UINT16*) (((SIZE_T) p_headerinfo) + bytepos + 10u));
	pb_params.sample_rate = *((UINT32*) (((SIZE_T) p_headerinfo) + bytepos + 12u));

	bit_depth = *((UINT16*) (((SIZE_T) p_headerinfo) + bytepos + 22u));
	...
	while(TRUE)
	{
		...
		if(compare_signature("data", (const CHAR*) (((SIZE_T) p_headerinfo) + bytepos))) break;
