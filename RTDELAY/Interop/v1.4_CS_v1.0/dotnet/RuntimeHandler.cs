...
public static class RuntimeHandler
{
	...
	public static void Proc_LoadFile_CreateAudioObj()
	{
		if(CPPCore.LoadFile_CreateAudioObject() == 0)
		{
	...
	public static void Proc_InitAudioObj_RunPlayback()
	{
		if(CPPCore.InitializeAudioObject() == 0)
		{
			Win32Aux._MessageBox(CPPCore.GetLastErrorMessage(), "ERROR", (Win32Aux.MB_ICONEXCLAMATION | Win32Aux.MB_OK));
			return;
		}
	...
	public static bool AttemptUpdateParam(string inputText, int param, int nFX)
	{
		...
		{
			case ATTEMPTUPDATEPARAM_DRYINAMP:
				if(CPPCore.SetDryInputAmplitude(f32) != 0) return true;
				break;

			case ATTEMPTUPDATEPARAM_OUTAMP:
				if(CPPCore.SetOutputAmplitude(f32) != 0) return true;
				break;

			case ATTEMPTUPDATEPARAM_FFDELAY:
				if(CPPCore.SetFFDelay((nuint) nFX, (nuint) i32) != 0) return true;
				break;

			case ATTEMPTUPDATEPARAM_FFAMP:
				if(CPPCore.SetFFAmplitude((nuint) nFX, f32) != 0) return true;
				break;

			case ATTEMPTUPDATEPARAM_FBDELAY:
				if(CPPCore.SetFBDelay((nuint) nFX, (nuint) i32) != 0) return true;
				break;

			case ATTEMPTUPDATEPARAM_FBAMP:
				if(CPPCore.SetFBAmplitude((nuint) nFX, f32) != 0) return true;
				break;
