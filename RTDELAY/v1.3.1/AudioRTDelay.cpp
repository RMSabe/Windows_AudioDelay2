...
BOOL WINAPI AudioRTDelay::runDSP(ULONG_PTR n_segment)
{
...
FLOAT* WINAPI AudioRTDelay::getInputBufferSegment(ULONG_PTR n_segment)
{
	if(this->status < 1) return NULL;
	if(n_segment >= this->BUFFER_N_SEGMENTS)
	{
...
FLOAT* WINAPI AudioRTDelay::getOutputBufferSegment(ULONG_PTR n_segment)
{
	if(this->status < 1) return NULL;
	if(n_segment >= this->BUFFER_N_SEGMENTS)
	{
...
BOOL WINAPI AudioRTDelay::getFFParams(ULONG_PTR n_fx, audiortdelay_fx_params_t *p_params)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioRTDelay::getFBParams(ULONG_PTR n_fx, audiortdelay_fx_params_t *p_params)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioRTDelay::setFFDelay(ULONG_PTR n_fx, ULONG_PTR delay)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioRTDelay::setFFAmplitude(ULONG_PTR n_fx, FLOAT amp)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioRTDelay::setFBDelay(ULONG_PTR n_fx, ULONG_PTR delay)
{
	if(this->status < 1) return FALSE;
...
BOOL WINAPI AudioRTDelay::setFBAmplitude(ULONG_PTR n_fx, FLOAT amp)
{
	if(this->status < 1) return FALSE;
...
VOID WINAPI AudioRTDelay::deinitialize(VOID)
{
	this->status = this->STATUS_UNINITIALIZED;
	this->buffer_free();
...
BOOL WINAPI AudioRTDelay::buffer_alloc(VOID)
{
	...
	{
		this->err_msg = TEXT("AudioRTDelay::buffer_alloc: Error: failed to retrieve process heap.");
		return FALSE;
	}
	...
	{
		this->buffer_free();
		this->err_msg = TEXT("AudioRTDelay::buffer_alloc: Error: failed to allocate heap memory.");
		return FALSE;
	}
	...
	{
		this->pp_bufferinput_segments[n_seg] = (FLOAT*) (((ULONG_PTR) this->p_bufferinput) + n_seg*(this->BUFFER_SEGMENT_SIZE_BYTES));
		this->pp_bufferoutput_segments[n_seg] = (FLOAT*) (((ULONG_PTR) this->p_bufferoutput) + n_seg*(this->BUFFER_SEGMENT_SIZE_BYTES));
	}

	return TRUE;
}

BOOL WINAPI AudioRTDelay::buffer_free(VOID)
{
	HANDLE p_processheap = NULL;
	...
	if(p_processheap == NULL)
	{
		this->err_msg = TEXT("AudioRTDelay::buffer_free: Error: failed to retrieve process heap.");
		return FALSE;
	}
	...
	if(this->p_bufferinput != NULL)
	{
		...
		{
			this->err_msg = TEXT("AudioRTDelay::buffer_free: Error: failed to release heap memory.");
			return FALSE;
		}
	...
	if(this->pp_bufferinput_segments != NULL)
	{
		if(!HeapFree(p_processheap, 0u, this->pp_bufferinput_segments))
		{
			this->err_msg = TEXT("AudioRTDelay::buffer_free: Error: failed to release heap memory.");
			return FALSE;
		}
	...
	if(this->pp_bufferoutput_segments != NULL)
	{
		if(!HeapFree(p_processheap, 0u, this->pp_bufferoutput_segments))
		{
			this->err_msg = TEXT("AudioRTDelay::buffer_free: Error: failed to release heap memory.");
			return FALSE;
		}
...
BOOL WINAPI AudioRTDelay::buffer_fxparams_alloc(VOID)
{
	...
	if(p_processheap == NULL)
	{
		this->err_msg = TEXT("AudioRTDelay::buffer_fxparams_alloc: Error: failed to retrieve process heap.");
		return FALSE;
	}
	...
	if(this->P_FF_PARAMS_LENGTH)
	{
		...
		if(this->p_ff_params == NULL)
		{
			...
			this->err_msg = TEXT("AudioRTDelay::buffer_fxparams_alloc: Error: failed to allocate heap memory.");
			return FALSE;
		}
	}
...
BOOL WINAPI AudioRTDelay::buffer_fxparams_free(VOID)
{
	HANDLE p_processheap = NULL;
	...
	if(p_processheap == NULL)
	{
		this->err_msg = TEXT("AudioRTDelay::buffer_fxparams_free: Error: failed to retrieve process heap.");
		return FALSE;
	}
	...
	if(this->p_ff_params != NULL)
	{
		...
		{
			this->err_msg = TEXT("AudioRTDelay::buffer_fxparams_free: Error: failed to release heap memory.");
			return FALSE;
		}
...
BOOL WINAPI AudioRTDelay::retrieve_prev_nframe(ULONG_PTR curr_buf_nframe, ULONG_PTR n_delay, ULONG_PTR *p_prev_buf_nframe, ULONG_PTR *p_prev_nseg, ULONG_PTR *p_prev_seg_nframe)
{
	ULONG_PTR prev_buf_nframe = 0u;
...
BOOL WINAPI AudioRTDelay::retrieve_prev_nframe(ULONG_PTR curr_nseg, ULONG_PTR curr_seg_nframe, ULONG_PTR n_delay, ULONG_PTR *p_prev_buf_nframe, ULONG_PTR *p_prev_nseg, ULONG_PTR *p_prev_seg_nframe)
{
